...
export const WrestlerMentionsModal = ({ wrestler, newsItems, onClose }: WrestlerMentionsModalProps) => {
  // Filter news items that mention this wrestler
  const wrestlerMentions = newsItems.filter(item => 
    item.title.toLowerCase().includes(wrestler.wrestler_name.toLowerCase()) ||
    (item.contentSnippet && item.contentSnippet.toLowerCase().includes(wrestler.wrestler_name.toLowerCase()))
  );
